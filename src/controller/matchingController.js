...
async function getMatchingInfo(req, res) {
    try {
        const matchingResult = await matchingService.getMatchingInfo(req.headers.authorization);
        if(matchingResult == -1) {
            console.log('토큰 오류');
            errResponse(res, returnCode.UNAUTHORIZED, '토큰 오류');
        } else {
...
async function getMatchingInfoPage(req, res) {
    try {
        const matchingResult = await matchingService.getMatchingInfoPage(req.headers.authorization, req.params.matchingIdx);
        if(matchingResult == -1) {
            console.log('토큰 오류');
            errResponse(res, returnCode.UNAUTHORIZED, '토큰 오류');
        } else if(matchingResult == -2) {
